...
export default function PrescriptionsPage() {
    ...
    const fetchData = async () => {
      try {
        const [prescriptionsRes, patientsRes] = await Promise.all([
          medicationsApi.getPrescriptions(),
          patientsApi.getAll()
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modal === 'create') {
        const response = await medicationsApi.createPrescription(formData);
        setPrescriptions([...prescriptions, response.data]);
      } else if (modal === 'edit' && selectedItem) {
        const response = await medicationsApi.updatePrescription(selectedItem.id, formData);
        setPrescriptions(prescriptions.map(p => p.id === selectedItem.id ? response.data : p));
      }
      closeModal();
    } catch (error) {
