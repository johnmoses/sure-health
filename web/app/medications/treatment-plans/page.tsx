...
export default function TreatmentPlansPage() {
    ...
    const fetchData = async () => {
      try {
        const [plansRes, patientsRes] = await Promise.all([
          medicationsApi.getTreatmentPlans(),
          patientsApi.getAll()
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modal === 'create') {
        const response = await medicationsApi.createTreatmentPlan(formData);
        setTreatmentPlans([...treatmentPlans, response.data]);
      } else if (modal === 'edit' && selectedItem) {
        const response = await medicationsApi.updateTreatmentPlan(selectedItem.id, formData);
